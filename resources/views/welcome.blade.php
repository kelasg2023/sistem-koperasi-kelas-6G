...
<div class="min-h-screen bg-[#F6F8F6] dark:bg-gray-950 font-sans flex flex-col justify-between">
   ...
   <header class="sticky top-0 z-50 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 shadow-sm px-4 md:px-12 py-3.5 flex items-center justify-between">
        ...
        <div class="flex items-center gap-4 relative">
           ...
           <a href="{{ route('keranjang.index') }}" class="relative text-gray-700 dark:text-gray-300 hover:text-[#1A622A] focus:outline-none mr-2 block" x-data>
    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
    </svg>
    
    {{-- Angka notifikasi keranjang --}}
    <span x-cloak x-show="$store.cart.count > 0" class="absolute -top-1.5 -right-2 bg-[#1A622A] text-white text-[9px] font-bold w-4 h-4 rounded-full flex items-center justify-center border border-white" x-text="$store.cart.count">
    </span>
</a>
    ...
    <div class="grid grid-cols-4 md:grid-cols-8 gap-4">
        ...
        <a href="{{ route('produk.kategori', $kat['id_kategori']) }}" class="flex flex-col items-center text-center cursor-pointer group">
            <div class="w-14 h-14 md:w-[72px] md:h-[72px] rounded-full flex items-center justify-center bg-[#E8F5EC] dark:bg-emerald-950/40 transition group-hover:scale-105">
                <span class="text-2xl md:text-3xl group-hover:-translate-y-1 transition-transform">🛒</span>
            </div>
            <span class="text-[11px] font-semibold text-gray-800 dark:text-gray-300 mt-2.5">{{ $kat['nama_kategori'] }}</span>
        </a>
        @empty
        @php
            $kategorisFallback = [
        ...
        <a href="{{ route('produk.index') }}" class="flex flex-col items-center text-center cursor-pointer group">
            <div class="w-14 h-14 md:w-[72px] md:h-[72px] rounded-full flex items-center justify-center bg-[#E8F5EC] dark:bg-emerald-950/40 transition group-hover:scale-105">
                <span class="text-2xl md:text-3xl group-hover:-translate-y-1 transition-transform">{{ $kat['icon'] }}</span>
            </div>
            <span class="text-[11px] font-semibold text-gray-800 dark:text-gray-300 mt-2.5">{{ $kat['label'] }}</span>
        </a>
        @endforeach
        @endforelse
    </div>
        ...
        <section id="promo-hari-ini" class="mt-12 scroll-mt-24">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                @forelse(array_slice($promos ?? [], 0, 4) as $promo)
                @php
                    $hargaAsli = (float) ($promo['harga'] ?? 0);
                    $diskon = (float) ($promo['diskon_persen'] ?? 0);
                    $hargaDiskon = $hargaAsli * (1 - $diskon / 100);
                @endphp
                <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-4 relative flex flex-col justify-between shadow-sm hover:shadow-md transition">
                    <div>
                        ...
                        <h4 class="text-xs font-bold text-gray-800 dark:text-gray-200 mb-1.5 line-clamp-1">{{ $promo['nama'] }}</h4>
                        <div class="flex items-baseline gap-1.5 mb-2 flex-wrap">
                            <span class="text-sm font-extrabold text-red-600">Rp {{ number_format($hargaDiskon, 0, ',', '.') }}</span>
                            <span class="text-[10px] text-gray-400 line-through">Rp {{ number_format($hargaAsli, 0, ',', '.') }}</span>
                        </div>
                    </div>
                    <div>
                        <div class="flex justify-between text-[10px] text-gray-500 mb-1">
                            <span>Sisa {{ $promo['stok'] ?? 0 }} item</span>
                            <span>Tersedia</span>
                        </div>
                        <div class="w-full bg-gray-100 dark:bg-gray-700 h-1.5 rounded-full overflow-hidden mb-4">
                            @php $stokRatio = min(100, (($promo['stok'] ?? 10) / 50) * 100); @endphp
                            <div class="bg-red-600 h-full rounded-full" style="width: {{ $stokRatio }}%"></div>
                        </div>
                        <a href="{{ route('produk.detail', $promo['barang_id']) }}" class="block text-center w-full py-2 bg-[#2D7A42] hover:bg-[#1E5C2F] text-white text-xs font-bold rounded-lg transition">
                            Beli Sekarang
                        </a>
                    </div>
                </div>
                ...
                <div class="col-span-4 text-center py-10 text-gray-500">
                    Belum ada promo spesial hari ini.
                </div>
                @endforelse
            </div>
        </section>
    ...
    <section class="bg-[#1A1A1A] dark:bg-black py-12 px-4 sm:px-6 md:px-12 -mx-4 sm:-mx-6 md:-mx-12 mt-12 transition-colors">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                @forelse(array_slice($barangs ?? [], 0, 4) as $b)
                @php
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl p-4 relative flex flex-col justify-between shadow-md hover:shadow-xl transition group">
                    ...
                    <div>
                        <div class="w-full h-36 flex items-center justify-center bg-gray-50 dark:bg-gray-700/30 rounded-xl overflow-hidden mb-3.5 relative text-6xl text-center group-hover:scale-105 transition">
                            🛒
                            <div class="absolute bottom-2 left-2 flex gap-1 z-10">
                                <span class="bg-[#E8F5EC] dark:bg-emerald-950/50 text-[#2D7A42] dark:text-emerald-400 text-[9px] font-extrabold px-1.5 py-0.5 rounded">{{ strtoupper($b['kategori']['nama_kategori'] ?? 'PRODUK') }}</span>
                                <span class="bg-[#F5820A] text-white text-[9px] font-extrabold px-1.5 py-0.5 rounded">Harga Anggota</span>
                            </div>
                        </div>
                        <h4 class="text-sm font-bold text-gray-900 dark:text-gray-100 line-clamp-1">{{ $b['nama'] }}</h4>
                        <span class="text-[10px] text-gray-400 dark:text-gray-500 block mb-3">Sisa stok: {{ $b['stok'] }}</span>
                    </div>
                    <div class="flex items-center justify-between mt-auto">
                        <div>
                            <div class="text-base font-extrabold text-[#2D7A42] dark:text-emerald-400">Rp {{ number_format($hargaDiskon, 0, ',', '.') }}</div>
                            @if($diskon > 0)
                                <div class="text-[9px] text-gray-400 dark:text-gray-500 line-through">Rp {{ number_format($hargaAsli, 0, ',', '.') }}</div>
                            @endif
                        </div>
                        <a href="{{ route('produk.detail', $b['barang_id']) }}" class="w-8 h-8 rounded-full bg-[#F5820A] hover:bg-orange-600 text-white flex items-center justify-center shadow transition active:scale-90 focus:outline-none">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
