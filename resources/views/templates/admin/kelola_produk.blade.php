...
<div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Atur dan kelola semua produk koperasi.</p>
        </div>
...
<script>
    // ==================== KONFIGURASI ====================
    const API_BASE_READ  = '/api-proxy/barang';        // GET list & show
    const API_BASE_WRITE = '/api-proxy/admin/barang';   // POST, PUT, DELETE
        ...
        pageItems.forEach((p, index) => {
            ...
            tr.innerHTML = `
                ...
                <td class="py-3 pr-4 text-right">
                    <button onclick="editProduk(${p.id})" class="text-orange-500 hover:text-orange-600 mr-3">
                        <i class="fa-solid fa-pen"></i>
                    </button>
                    <button onclick="deleteProduk(${p.id})" class="text-red-500 hover:text-red-600">
    ...
    function editProduk(id) {
        const p = allProduk.find(item => item.id == id);
        if (!p) return;

        document.getElementById('modalTitle').textContent = 'Edit Produk';
        document.getElementById('produkId').value = p.id;
        document.getElementById('nama').value = p.nama;
        document.getElementById('id_kategori').value = p.id_kategori;
        document.getElementById('stok').value = p.stok;
        document.getElementById('harga').value = p.harga;
        document.getElementById('diskon_persen').value = p.diskon_persen ?? '';
        document.getElementById('deskripsi').value = p.deskripsi ?? '';

        // Foto lama tidak bisa diisi otomatis ke <input type="file"> karena
        // pembatasan keamanan browser. User harus unggah ulang kalau ingin ganti foto.
    ...
    async function submitProduk(e) {
        ...
        // Laravel tidak bisa memproses file upload lewat method PUT asli via fetch,
        // jadi method di-spoof lewat field _method dan tetap dikirim sebagai POST.
        // POST ke /admin/barang untuk create, atau /admin/barang/{id} untuk update (method-spoofed)
        const url = id ? `${API_BASE_WRITE}/${id}` : API_BASE_WRITE;
        if (id) {
        ...
        try {
            ...
            Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: id ? 'Produk diperbarui!' : 'Produk ditambahkan!', showConfirmButton: false, timer: 2000 });
            closeModal();
            await loadProduk();
        } catch (err) {
            console.error(err);
    ...
    async function deleteProduk(id) {
        ...
        try {
            const res = await fetch(`${API_BASE_WRITE}/${id}`, {
                method: 'DELETE',
                headers: {
                    'Accept': 'application/json',
                    'X-CSRF-TOKEN': getCsrfToken(),
            ...
            if (!res.ok || !json.success) {
                Swal.fire('Gagal', json.message || 'Gagal menghapus produk.', 'error');
                return;
            }
